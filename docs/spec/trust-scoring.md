...
Trust recovery is intentionally asymmetric: trust loss is tier-scaled: 3x at T0 through 10x at T7 (P(T)=3+T). This mirrors real-world trust dynamics where a single failure can undo many successes.
...
**Asymmetry rationale:** A `policy_violation` signal costs -50 points instantly. Recovering those 50 points requires at least 3 perfect (value = 1.0) success signals at base rate — roughly a 10:1 loss-to-gain ratio.
...
The system uses **graduated per-tier hysteresis bands**: `[25, 25, 20, 20, 15, 10, 10, 10]` — wider at lower tiers (where new agents fluctuate more), tighter at the top (where stable agents operate).

| Tier | Hysteresis Buffer | Transition Behavior |
|------|------------------|---------------------|
| **T0** | 25 points | Wider buffer for new agents |
| **T1** | 25 points | Wider buffer for new agents |
| **T2** | 20 points | Moderate buffer |
| **T3** | 20 points | Moderate buffer |
| **T4** | 15 points | Narrowing buffer |
| **T5** | 10 points | Tight buffer for stable agents |
| **T6** | 10 points | Tight buffer for stable agents |
| **T7** | 10 points | Tight buffer for stable agents |

**Promotion** is immediate for T0-T4 (logarithmic gain is the natural throttle). T5 requires 7 days, T6 requires 10 days, T7 requires 14 days at the previous tier. **Demotion** is delayed by the per-tier hysteresis buffer.

**Example:** An entity at T4 Standard (min 650, hysteresis 15) will:
- **Retain T4** if score drops to 638 (within grace zone: 650 - 15 = 635)
- **Be demoted** if score drops to 630 (below grace zone threshold of 635)

```
Tier boundary:     |-------- T4 Standard (650+) --------|
Grace zone:        |=== 635-649 (retain T4) ===|
Demotion triggers: |--- below 635 (demote) ---|
```

**Design rationale:** Promotion is immediate at lower tiers to reward positive behavior without delay — the logarithmic gain formula already throttles progression naturally. At T5 and above, minimum observation periods (7/10/14 days) ensure the system has sufficient behavioral history before granting cross-system operations, multi-agent coordination, or full autonomy. Demotion is delayed by per-tier hysteresis to give agents a window to recover from transient score drops, reducing unnecessary capability revocation and re-granting churn.
...
All entities initialize at score 0 in PROVISIONING state. Agents must pass a qualification course (31+ exercises across 9 canary categories) to reach T1 Observed (score 200). Trust is earned through demonstrated capability, not granted at registration.
...
**REQ-TRS-006**: Recovery MUST apply asymmetrically — trust loss rate MUST exceed gain rate by P(T) = 3 + T (minimum 3x at T0, maximum 10x at T7).
...
**REQ-TRS-009**: Demotion MUST apply graduated per-tier hysteresis bands `[25, 25, 20, 20, 15, 10, 10, 10]` — entities MUST NOT be demoted until score falls below `(tier_minimum - hysteresis[tier])`.
